...
export class FakeWorker extends EventTarget {
  ...
  async #load_worker() {
    ...
    let temp_script = `
      ${loader.runtime_src}
      wool_frame.loader.set_url("${loader.url}");
      wool_frame.loader.set_frame_id("${worker_id}");
      wool_frame.context.update_ctx();
      wool_frame.rpc.set_on_attach(() => {
        wool_frame.context.run_script(${worker_js});
      })
    `;
    ...
    for (let url of recorded_urls) {
      ...
      cache_puts.push(`wool_frame.network.cache_put(${safe_url}, ${safe_contents});`);
    }
    let cache_substr = cache_puts.join("\n");
    let real_script = `
      ${loader.runtime_src}
      ${cache_substr}
      wool_frame.loader.set_url(${JSON.stringify(loader.url)});
      wool_frame.loader.set_frame_id("${worker_id}");
      wool_frame.network.enable_network();
      wool_frame.context.update_ctx();
      wool_frame.rpc.set_on_attach(() => {
        wool_frame.context.run_script(${worker_js});
      })
