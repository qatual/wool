import * as wool from "../dist/wool.mjs";
...
const main_frame = new wool.controller.ProxyFrame();
globalThis.wool = wool;
...
main_frame.on_load = async () => {
  ...
  if (!favicon_url.startsWith("data:")) {
    let response = await wool.libcurl.fetch(favicon_url);
...
async function navigate_clicked() {
  ...
  if (!url.startsWith("http:") && !url.startsWith("https:") && !url.startsWith("wool:")) 
...
function toggle_options() {
  ...
  //apply options
  wool.libcurl.set_websocket(wisp_url_input.value);
}
...
async function create_homepage() {
  ...
  html.getElementById("wool_version").textContent = version_text.textContent;

  let homepage_html = "<!DOCTYPE html>" + html.documentElement.outerHTML;
  main_frame.special_pages["wool://home"] = homepage_html;
}
...
async function main() {
  ...
  if (location.hostname.endsWith(".pages.dev") || (location.protocol !== "http:" && location.protocol !== "https:")) 
    wool.libcurl.set_websocket(wisp_url);
  else {
    wisp_url = location.origin.replace("http", "ws");
    wool.libcurl.set_websocket(wisp_url);
  }

  version_text.textContent = `v${wool.version.ver} (${wool.version.hash})`;
